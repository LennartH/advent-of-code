...
-- 2025-08-02
--    ASOF: 0.57257 +- 0.00294 seconds (+- 0.51%)
--     ^-- dedicated table for walls: 0.50578 +- 0.00433 seconds (+- 0.86%)
--    CASE: 0.15272 +- 0.00349 seconds (+- 2.29%)
-- |---^-- dedicated table for walls: 0.13325 +- 0.00229 seconds (+- 1.72%)
-- | mathy: 0.21263 +- 0.00365 seconds (+- 1.72%)
-- |   ^-- dedicated table for walls: 0.18893 +- 0.00389 seconds (+- 2.06%)
-- |
-- > raywalking: 0.13085 +- 0.00285 seconds (+- 2.18%)
--    ^-- where instead of join for start position: 0.12623 +- 0.00327 seconds (+- 2.59%)
...
--         --     ASOF INNER JOIN walls w ON w.x = s.x AND w.y < s.y
...
--         --     ASOF INNER JOIN walls w ON w.y = s.y AND w.x > s.x
--         --     SELECT s.*, w.x as wall_x, w.y as wall_y
--         --     WHERE s.dir = '>'
...
--             INNER JOIN walls w ON CASE dir
...
-- 2025-08-02
--   early pathfinding termination: 1.83807 +- 0.00937 seconds (+- 0.51%)
--    ^-- obstacle building from precalculated steps: 1.8393 +- 0.0148 seconds (+- 0.81%)
--    ^---- summarize on all records: {"column_name":"it","column_type":"INTEGER","min":"0","max":"116","approx_unique":117,"avg":"8.14310208388288","std":"16.24402178308896","q25":"1","q50":"2","q75":"6","count":22746,"null_percentage":0.00}
--    ^-- summarize max(it) by index: {"column_name":"max(it)","column_type":"INTEGER","min":"0","max":"116","approx_unique":52,"avg":"4.132220216606498","std":"7.898797347574062","q25":"1","q50":"2","q75":"5","count":4432,"null_percentage":0.00}
...
-- --             WHERE NOT EXISTS (FROM walls WHERE y = obstacle_y AND x = obstacle_x)
-- --             QUALIFY row_number() OVER (PARTITION BY obstacle_y, obstacle_x ORDER BY index) = 1
-- --         ),
-- --         obstacles_with_context AS (
-- --             FROM obstacles o
-- --             LEFT JOIN ray_steps p ON p.step_index > o.step_index AND (
-- --             -- JOIN guard_path p ON p.step_index >= o.step_index AND (
-- --                     abs(p.prev_y - obstacle_y) + abs(p.prev_x - obstacle_x) +
-- --                     abs(obstacle_y - p.y) + abs(obstacle_x - p.x)
-- --                 ) = abs(p.prev_y - p.y) + abs(p.prev_x - p.x)
-- --             SELECT
-- --                 o.*,
...
--             -- LEFT JOIN obstacles_with_path o ON o.obstacle_y = t.y AND o.obstacle_x = t.x
--             SELECT
--                 t.y, t.x, t.symbol,
...
-- 2025-07-24
--   precalculated steps: 0.19348 +- 0.00146 seconds (+- 0.75%)
--    ^-- wall_hits not materialized: 0.21892 +- 0.00223 seconds (+- 1.02%)
--    ^-- walls not materialized: 0.21279 +- 0.00292 seconds (+- 1.37%)
--    ^-- walls and wall_hits not materialized: 0.25566 +- 0.00801 seconds (+- 3.13%)
--         ^-- from tiles != #: 0.20190 +- 0.00203 seconds (+- 1.01%)
--         ^-- wall not exists from tiles: 0.22175 +- 0.00325 seconds (+- 1.47%)
--         ^-- not wall exists from tiles: 0.22577 +- 0.00389 seconds (+- 1.72%)
--    ^-- from tiles != #: 0.20109 +- 0.00441 seconds (+- 2.19%)
--    ^-- wall not exists from tiles: 0.19462 +- 0.00258 seconds (+- 1.32%)
--    ^-- not wall exists from tiles: 0.19432 +- 0.00272 seconds (+- 1.40%)
--    ^-- separate table for walls: 0.19380 +- 0.00298 seconds (+- 1.54%)
-- 2025-07-27
...
-- |--^-- fewer directions joins: 0.17608 +- 0.00220 seconds (+- 1.25%)
-- |
-- > precalculated steps: 0.17608 +- 0.00220 seconds (+- 1.25%)
--    ^-- wall_moves join with walls instead of wall_hits: 0.16043 +- 0.00310 seconds (+- 1.93%)
--         ^-- result: 4419, expected: 4433
-- |-------^-- wall_hits not materialized: 0.15472 +- 0.00282 seconds (+- 1.82%)
-- |
-- > precalculated steps: 0.15472 +- 0.00282 seconds (+- 1.82%)
...
-- 2025-07-26
--   [BROKEN] loop detection without context: 1.5231 +- 0.0163 seconds (+- 1.07%)
--    ^-- result: 1513, expected: 1516 | obstacle hits do not register when map would be exited (filtered by left join)
--   loop detection without context: 1.7594 +- 0.0103 seconds (+- 0.59%)
-- 2025-07-27
--   loop detection without context: 1.7592 +- 0.0122 seconds (+- 0.69%)
--    ^-- [BROKEN] obstacle_hit via CASE: 1.6605 +- 0.0126 seconds (+- 0.76%)
--         ^-- result: 1574, expected: 1516
--         v-- Times are a bit wonky (up to 0.2s diff between runs)
-- |--^-- exits in precalculated moves: 1.5864 +- 0.0186 seconds (+- 1.17%)
-- |       ^-- obstacle_hit via CASE: 1.6021 +- 0.0133 seconds (+- 0.83%)
-- > loop detection (no context): 1.5586 +- 0.0101 seconds (+- 0.65%)
--    ^-- tuple for current tile: 1.5547 +- 0.0102 seconds (+- 0.66%)  |  commit 86da4bba301d36e247930ac0133a2f3ee9f9b3cf
--    ^-- [BROKEN] without initial_steps CTE: 1.5667 +- 0.0149 seconds (+- 0.95%)
--         ^-- result: 1517, expected: 1516 | first obstacle always loops, because wall_moves contains (start) -> (first_wall)
-- |--^-- without initial_steps CTE: 1.5461 +- 0.0116 seconds (+- 0.75%)
-- |
-- > loop detection (no context): 1.6377 +- 0.0196 seconds (+- 1.20%)
--    ^-- with previous tiles of original path: 1.6731 +- 0.0117 seconds (+- 0.70%)
--    ^-- [BROKEN] with previous & future tiles of original path: 0.86507 +- 0.00921 seconds (+- 1.06%)
--         ^-- result: 1473, expected: 1516
--    ^-- [less broken] with previous & future tiles of original path: 1.3155 +- 0.0129 seconds (+- 0.98%)
--         ^-- works for input, but not for example
-- 2025-07-28
--   loop detection (no context): 1.40994 +- 0.00805 seconds (+- 0.57%)
...
-- 2025-07-28
--   loop detection (no context): 1.4593 +- 0.0127 seconds (+- 0.87%)
--   [less broken] with previous & future tiles of original path: 1.2543 +- 0.0113 seconds (+- 0.90%)
--    ^-- [BROKEN] context building from raywalking approach: 1.1177 +- 0.0114 seconds (+- 1.02%)
--         ^-- result: 1517, expected: 1516
--         ^-- in dedicated table: 1.16229 +- 0.00737 seconds (+- 0.63%)
-- 2025-08-02
--   loop detection (no context): 1.4850 +- 0.0147 seconds (+- 0.99%)
--   [less broken] with previous & future tiles of original path: 1.2436 +- 0.0112 seconds (+- 0.90%)  |  commit 86da4bba301d36e247930ac0133a2f3ee9f9b3cf
--    ^-- [BROKEN] context building from raywalking approach: 1.11547 +- 0.00730 seconds (+- 0.65%)  |  commit 86da4bba301d36e247930ac0133a2f3ee9f9b3cf
--         ^-- result: 1517, expected: 1516
