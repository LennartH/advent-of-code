...
WITH RECURSIVE
    ...
    -- deep_dirpathss AS (
        WITH RECURSIVE
            ...
            -- TODO Adjust dir_lookup to be already unpacked, so the histogram doesn't have be packed/unpacked so often?
            supercede AS (
                SELECT
            ...
            reduce AS (
                ...
                SELECT
                    ...
                    rank() OVER (
                        PARTITION BY drill_id, id
                        ORDER BY list_sum(map_values(hist))
                    ) as rank, -- TODO is this really pruning?
                FROM collect
                WHERE pos = length
                QUALIFY rank = 1
            )
...
--             QUALIFY rank = 1
        ...
        SELECT
            ...
            hist,
        FROM reduce
        WHERE depth < 25 -- TODO Maybe my server is strong enough?
    )
    )
...
-- union all
-- from deep_dirpathss
-- order by depth, drill_id, id
-- order by depth, drill_id, id, pos
-- order by depth, drill_id, id, pos, branch
-- ;
...
CREATE OR REPLACE VIEW sequences AS (
    WITH RECURSIVE
        ...
        dirpresses AS (
            ...
            LEFT JOIN deep_dirpaths USING (d1, d2)
...
--                 d1, d2,
--                 flatten(list(moves ORDER BY pos)) as moves,
--                 sum(len(moves)) as total,
--             FROM (
--                 SELECT
--                     d2.depth + d3.depth as depth,
--                     from_d1 as d1, 
--                     to_d2 as d2,
--                     pos,
--                     coalesce(moves, ['A']) as moves,
...
--                     WHERE depth = 2
--                 ) d3
--                 LEFT JOIN depth13 d2 ON d2.depth = 2 AND d2.d1 = d3.d1 AND d2.d2 = d3.d2
--             )
--             GROUP BY d1, d2
--         )
