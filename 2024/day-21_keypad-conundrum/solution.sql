...
--         -- FROM (VALUES
--         --     (2, 22, 17, MAP{5: 1, 20: 1, 12: 1, 4: 1, 19: 1, 18: 1, 0: 1, 16: 1}::MAP(INTEGER, HUGEINT))
--         -- ) _(depth, drill_id, id, hist)
--     ),
--     deep_dirpathss AS (
--         WITH RECURSIVE
--             explode AS (
...
--                         unnest(map_entries(hist), recursive := true), -- FIXME this loses 0s
--                         cardinality(hist)::INTEGER as length,
--                     FROM deep_dirpaths
--                 )
--             ),
--             -- TODO Adjust dir_lookup to be already unpacked, so the histogram doesn'd2 have be packed/unpacked so often?
--             supercede AS (
--                 SELECT
--                     any_value(depth) as depth,
...
--             ),
--             reduce AS (
--                 -- TODO Only keep shortest permutations
--                 SELECT
--                     depth,
--                     drill_id,
--                     id,
...
--                     -- ) as rank,
--                     list_sum(map_values(hist)) as total,
--                     rank() OVER (PARTITION BY id ORDER BY total) as rank,
--                 FROM collect
--                 WHERE pos = length
--                 QUALIFY rank = 1
--             )
...
--         --     hist,
--         -- FROM reduce
--         -- WHERE depth < 3 -- 25 -- TODO Maybe my server is strong enough?
--     )
--     -- )


-- from deep_dirpathss d
-- ;
...
--     d.length,
--     d.meta_id,
--     lm.symbol as meta_symbol,
--     d.meta_count,
-- from deep_dirpathss d
-- left join dir_id_lookup l ON l.id = d.id
-- left join dir_id_lookup lm ON lm.id = d.meta_id
-- order by d.depth, d.drill_id, d.id, d.pos
-- ;
...
--     l.d2,
--     p.hist,
-- FROM deep_dirpaths p
-- JOIN tracked_lookup l USING (drill_id)
-- -- QUALIFY row_number() OVER (PARTITION BY p.depth, l.id ORDER BY list_sum(map_values(p.hist))) = 1
-- -- delete me
-- -- order by depth, p.drill_id, l.id
